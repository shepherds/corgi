...
-----------------*/
function validatHttpResponse(item, body, res){
  if (item.expected){
    if (res.statusCode != item.expected.statuscode){
      return 'FAILED! expected status code :' + item.expected.statuscode +
        ' at ' + item.url + ' but got ' + res.statusCode;
    }
    else if (item.expected.contains && (!body ||
        (body.indexOf(item.expected.contains)==-1))){
      return 'FAILED! expected text "' + item.expected.contains +
        '" but it wasn\'t found';
    }
    else{
      return ''; //ok
